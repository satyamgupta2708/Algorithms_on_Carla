...
TOTAL_RUN_TIME         = 25.00 # game seconds (total runtime before sim end)
...
def read_waypoints(WAYPOINTS_FILENAME):
...
def exec_waypoint_nav_demo(args,waypoints,waypoints_np):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if  dist_to_last_waypoint < DIST_THRESHOLD_TO_LAST_WAYPOINT and current_speed < 10**-3:
        ...
        return y_history,speed_history
def main():
    """Main function.
    ...
    while True:
        try:
            ...
            # WAYPOINTS_FILENAME = 'speedtest_25mi.txt' 
            # WAYPOINTS_FILENAME = 'speedtest_31mi.txt' 
            # WAYPOINTS_FILENAME = 'speedtest_36mi.txt' 
            WAYPOINTS_FILENAME = 'speedtest_40mi.txt' 
            
            waypoints_np = None
            waypoints = read_waypoints(WAYPOINTS_FILENAME)
            stopping_distance = 18
            waypoints[-1][1] = -1*(np.abs(waypoints[-2][1]) + stopping_distance)
             
            # waypoints[-1][1] = -26.03
            waypoints_np = np.array(waypoints)
            y_history,speed_history = exec_waypoint_nav_demo(args,waypoints,waypoints_np)

         
            exceeding_distance = np.abs(y_history[-1]) - np.abs(waypoints[-1][1])
            print("max speed achieved in miles/hr",max(speed_history)*2.2369)
            print('exceeding distance', exceeding_distance)
            print('Done.')
            return
